...
const functions = [
  ...
  "rollingTemplateSessionTokenV24",
  "v5DefaultAutoRange",
...
assert.equal(firstRollingRoot.templateId, rollingTemplate.id, "신규 회차가 업무 DB 연결정보를 상속해야 합니다.");
assert.equal(firstRollingRoot.seriesId, `dbseries:${rollingTemplate.id}`, "신규 회차가 원본 반복 묶음 식별자를 가져야 합니다.");
assert.equal(firstRollingRoot.urgent, true, "신규 회차가 긴급 여부를 상속해야 합니다.");
assert.equal(firstRollingRoot.end, "2026-09-17", "신규 회차가 기간 일수를 상속해야 합니다.");
assert.equal(firstRollingRoot.deadline, "2026-09-19", "신규 회차가 기한 오프셋을 상속해야 합니다.");
assert.deepEqual(Array.from(firstRollingRoot.checklist, (item) => [item.text, item.done]), [["원본 확인", false]], "신규 회차가 체크리스트를 미완료 상태로 상속해야 합니다.");
assert.equal(firstRollingRoot.notes, "원본 비고", "신규 회차가 원본 비고를 상속해야 합니다.");
...
assert.deepEqual(Array.from(firstRollingChildren, (task) => task.start), ["2026-09-21", "2026-09-23"], "새 회차의 연계 단계가 기존 근무일·달력일 규칙대로 계산되어야 합니다.");
assert.deepEqual(Array.from(firstRollingChildren, (task) => task.checklist[0]?.text), ["2단계 확인", "3단계 확인"], "새 회차의 연계 체크리스트가 단계별로 상속되어야 합니다.");
assert.equal(new Set(context.state.tasks.map((task) => task.generatedKey)).size, context.state.tasks.length, "루트와 연계 단계 모두 generatedKey 중복이 없어야 합니다.");
assert.equal(sandbox.saveCalls.length, 1, "최초 Rolling 보충은 한 번의 상태 저장으로 묶어야 합니다.");

firstRollingRoot.name = "사용자가 수정한 업무명";
firstRollingRoot.owner = "사용자 지정 담당자";
firstRollingRoot.start = "2026-09-24";
firstRollingRoot.end = "2026-09-26";
firstRollingRoot.deadline = "2026-09-28";
firstRollingRoot.status = "done";
firstRollingRoot.actualComplete = "2026-09-18";
firstRollingRoot.manualOverride = true;
firstRollingRoot.scheduleOverride = true;
...
assert.equal(sandbox.saveCalls.length, 1, "변경 없는 반복 검사는 Firestore 저장을 예약하면 안 됩니다.");
...
assert.equal(sandbox.saveCalls.length, 2, "다음 달 보충도 한 번만 저장하고 반복 검사에서는 저장하지 않아야 합니다.");

const lastRollingRoot = context.state.tasks.find((task) => task.generatedKey === "dbauto:qa-rolling-template:M:2027-10");
const lastRollingChildren = context.state.tasks.filter((task) => task.parentAutoRootId === lastRollingRoot.id).sort((a, b) => a.step - b.step);
const preservedExistingChild = JSON.stringify(lastRollingChildren[0]);
const missingChild = lastRollingChildren[1];
context.state.tasks = context.state.tasks.filter((task) => task.id !== missingChild.id);
assert.equal(context.ensureAutoSchedulesV5(), 0, "기존 루트의 누락 연계단계 복구를 새 루트 회차로 계산하면 안 됩니다.");
assert.equal(context.state.tasks.filter((task) => task.generatedKey === missingChild.generatedKey).length, 1, "부분 저장 실패로 빠진 연계단계만 다음 검사에서 복구해야 합니다.");
assert.equal(JSON.stringify(context.state.tasks.find((task) => task.id === lastRollingChildren[0].id)), preservedExistingChild, "누락 단계 복구 중 기존 연계단계를 수정하면 안 됩니다.");
assert.equal(JSON.stringify(firstRollingRoot), protectedRoot, "누락 단계 복구 중 과거 완료·수동 일정을 건드리면 안 됩니다.");

const lastRootIds = new Set([lastRollingRoot.id, ...context.state.tasks.filter((task) => task.parentAutoRootId === lastRollingRoot.id).map((task) => task.id)]);
context.state.tasks = context.state.tasks.filter((task) => !lastRootIds.has(task.id));
assert.equal(context.ensureAutoSchedulesV5(), 1, "확보 표식이 남아 있어도 실제로 빠진 미래 회차는 다음 검사에서 복구해야 합니다.");
assert.equal(context.state.tasks.filter((task) => task.generatedKey === lastRollingRoot.generatedKey).length, 1, "누락 루트 복구 뒤 같은 예정일 회차는 한 건이어야 합니다.");
assert.equal(context.ensureAutoSchedulesV5(), 0, "누락 복구 뒤 재검사에서도 중복 생성되면 안 됩니다.");
...
const holidayTemplates = [
  ...
  ["next", "2026-10-06"],
  ["keep", "2026-10-03"],
].map(([holidayShift, expected]) => ({
  ...JSON.parse(JSON.stringify(rollingTemplate)),
  id: `qa-holiday-${holidayShift}`,
  name: `[QA] 휴일 ${holidayShift}`,
  exactDay: 3,
  holidayShift,
  linkedSteps: [],
...
context.todayISO = () => "2026-10-01";
context.state = {
  tasks: [],
  templates: holidayTemplates,
  holidays: [{ id: "qa-october-holiday", date: "2026-10-05", reason: "회사휴무" }],
  categories: ["주거복지"],
  owners: ["QA담당자"],
  settings: { suppressedAutoKeys: [] },
};
assert.equal(context.ensureAutoSchedulesV5(), 3, "공휴일 앞당김·뒤로 미룸·그대로 규칙별 회차를 생성해야 합니다.");
for (const template of holidayTemplates) {
  const task = context.state.tasks.find((item) => item.autoSourceTemplateId === template.id);
  assert.equal(task.start, template.expected, `${template.holidayShift} 공휴일 규칙을 그대로 적용해야 합니다.`);
}

const weeklyEndTemplate = {
  ...JSON.parse(JSON.stringify(rollingTemplate)),
  id: "qa-weekly-end-boundary",
  name: "[QA] 주간 종료일 경계",
  cycle: "weekly",
  weekday: 0,
  weekdays: [0],
...
context.state = {
  tasks: [],
  templates: [weeklyEndTemplate],
  holidays: [{ id: "qa-october-holiday", date: "2026-10-05", reason: "회사휴무" }],
  categories: ["주거복지"],
  owners: ["QA담당자"],
  settings: { suppressedAutoKeys: [] },
};
assert.equal(context.ensureAutoSchedulesV5(), 0, "휴일 보정 결과가 명시 종료일을 넘는 주간 회차는 생성하면 안 됩니다.");
assert.equal(context.state.tasks.length, 0, "명시 종료일 이후로 미뤄진 업무가 남으면 안 됩니다.");
